...
def _direct_reference_hits(value, pointer="", identity_location="VALUE", dictionary_key=None):
    hits = []
    if isinstance(value, list):
        for index, child in enumerate(value):
            if not isinstance(child, dict):
                hits.extend(_direct_reference_hits(child, f"{pointer}/{index}", identity_location, dictionary_key))
    elif isinstance(value, str):
        for token in _reference_tokens(value):
            hits.append({
                "location": pointer or "/", "token": token, "value": value,
                "identityLocation": identity_location, "dictionaryKey": dictionary_key,
            })
    return hits


def _walk_relevant_records(value, pointer="", stats=None, skip_direct_values=False):
    """Yield one contextual semantic record for matching keys and/or direct values."""
    stats = stats if stats is not None else defaultdict(int)
    if isinstance(value, dict):
        stats["objectsTraversed"] += 1
        stats["dictionaryKeysInspected"] += len(value)
        stats["dictionaryValuesTraversed"] += len(value)
        matched_keys = set()
        for key, child in value.items():
            key_pointer = f"{pointer}/{_pointer_token(key)}"
            key_hits = [
                {
                    "location": key_pointer, "token": token, "value": key,
                    "identityLocation": "KEY", "dictionaryKey": key,
                    "associatedValue": child,
                }
                for token in _reference_tokens(key)
            ]
            if not key_hits:
                continue
            ...
            if isinstance(child, dict):
                for child_key, grandchild in child.items():
                    if not isinstance(grandchild, dict):
                        value_hits.extend(_direct_reference_hits(
                            grandchild,
                            f"{key_pointer}/{_pointer_token(child_key)}",
                            "VALUE",
                            child_key,
                        ))
            else:
                value_hits = _direct_reference_hits(child, key_pointer, "VALUE", key)
            yield {
                "recordLocation": key_pointer,
                "containingMapLocation": pointer or "/",
                "containingMapKeys": list(value),
                "dictionaryKey": key,
                "associatedValue": child,
                "referenceLocations": key_hits + value_hits,
                "_value": child,
            }

        if not skip_direct_values:
            direct = []
            for key, child in value.items():
                if key not in matched_keys and not isinstance(child, dict):
                    direct.extend(_direct_reference_hits(
                        child,
            ...
            if direct:
                yield {
                    "recordLocation": pointer or "/",
                    "containingMapLocation": pointer or "/",
                    "containingMapKeys": list(value),
                    "dictionaryKey": None,
                    "associatedValue": value,
                    "referenceLocations": direct,
                    "_value": value,
                }
        for key, child in value.items():
            yield from _walk_relevant_records(
                child,
                f"{pointer}/{_pointer_token(key)}",
                stats,
                skip_direct_values=key in matched_keys and isinstance(child, dict),
            )
    elif isinstance(value, list):
        stats["listsTraversed"] += 1
        for index, child in enumerate(value):
            yield from _walk_relevant_records(child, f"{pointer}/{index}", stats)
...
def _scan_current_authority_artifact(relative, authority_row, overrides):
    ...
    if not path.is_file():
        return {
            "records": [], "parseErrors": ["artifact missing"],
            "kind": _authority_artifact_kind(relative), "traversal": {},
            "structuredTraversalComplete": False,
        }
    stat = path.stat()
    ...
    try:
        ...
        if path.suffix.lower() == ".jsonl":
            for line_number, raw_line in enumerate(content.splitlines(), 1):
                ...
                row = json.loads(line)
                records.extend(_walk_relevant_records(row, f"/line/{line_number}", traversal))
        elif path.suffix.lower() == ".json":
            text = content.decode("utf-8-sig")
            records.extend(_walk_relevant_records(json.loads(text), stats=traversal))
        else:
            for line_number, raw_line in enumerate(content.splitlines(), 1):
                ...
                if tokens:
                    records.append({
                        ...
                        "containingMapLocation": None,
                        "containingMapKeys": [],
                        "dictionaryKey": None,
                        "associatedValue": line.strip(),
                        "referenceLocations": [{"location": f"/line/{line_number}", "token": token, "value": line.strip(), "identityLocation": "VALUE", "dictionaryKey": None} for token in tokens],
                        "_value": {"text": line.strip()},
                    })
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
        errors.append(str(error))
    result = {
        "records": records, "parseErrors": errors, "kind": kind,
        "traversal": dict(traversal),
        "structuredTraversalComplete": kind != "STRUCTURED" or not errors,
    }
...
def _identity_location(record):
    locations = {hit.get("identityLocation") for hit in record.get("referenceLocations", [])}
    if locations == {"KEY", "VALUE"}:
        return "BOTH"
    if "KEY" in locations:
        return "KEY"
    return "VALUE"
...
def _path_projection_scope(value, key_discovered=False):
    if isinstance(value, list) and key_discovered and all(isinstance(item, str) and "/" in item for item in value):
        return "EXACT_CURRENT_PATHS"
    if not isinstance(value, dict):
        return None
    for field, scope in (
        ("exactCurrentPaths", "EXACT_CURRENT_PATHS"), ("paths", "EXACT_CURRENT_PATHS"),
        ("exactTargetPaths", "EXACT_TARGET_PATHS"), ("allowedPaths", "ALLOWED_PATHS"),
        ("ownedPaths", "OWNED_PATHS"), ("referencePaths", "REFERENCE_PATHS"),
...
def _classify_authority_reference(relative, kind, location, value, record=None):
    record = record or {"referenceLocations": []}
    identity_location = _identity_location(record)
    key_discovered = identity_location in {"KEY", "BOTH"}
    associated = record.get("associatedValue") if key_discovered else value
    if kind != "STRUCTURED":
    ...
    if (
        relative == "00 Execution Control/FINAL_FREEZE_VALIDATION_RESULT.json"
        and location.startswith("/validationResult/derived/architectureAuthorityDiscovery/")
    ):
        return "REFERENCE_ONLY", "DERIVED_VALIDATOR_AUTHORITY_INVENTORY_EVIDENCE"
    fields = set(associated) if isinstance(associated, dict) else set()
    all_keys = _all_record_keys(associated)
    if _historical_record(associated):
        return "REFERENCE_ONLY", "HISTORICAL_RECORD_EXCLUDED_FROM_CURRENT_TOPOLOGY"
    if key_discovered and str(record.get("containingMapLocation") or "").endswith("/folderOwners") and isinstance(associated, str):
        return "OWNERSHIP_PROJECTION", "CANONICAL_SCOPED_OWNERSHIP_EQUALITY"
    if _path_projection_scope(associated, key_discovered):
        return "PATH_PROJECTION", "CANONICAL_SCOPED_PATH_EQUALITY"
    ...
    if key_discovered:
        return "UNCLASSIFIED_CURRENT_AUTHORITY", "NO_VALIDATION_RULE_FOR_KEYED_AUTHORITY"
    return "REFERENCE_ONLY", "STRUCTURED_REFERENCE_PRESENCE_AND_CURRENT_AUTHORITY"


def _scoped_reference_validation(classification, record, capabilities, tasks):
    tokens = sorted({hit["token"] for hit in record["referenceLocations"]})
    identity_location = _identity_location(record)
    associated = record.get("associatedValue") if identity_location in {"KEY", "BOTH"} else record.get("_value")
    if identity_location in {"KEY", "BOTH"}:
        capability_id = next((token for token in _reference_tokens(record.get("dictionaryKey")) if token.startswith("MR-CAP-")), None)
    else:
        explicit_identity = associated.get("capabilityId") if isinstance(associated, dict) else None
        capability_id = explicit_identity if explicit_identity in tokens else None
    capability = next((row for row in capabilities if row.get("capabilityId") == capability_id), None)
    primary_task_id = f"MR-IMPL-{capability_id.rsplit('-', 1)[-1]}" if capability_id else None
    task = next((row for row in tasks if row.get("taskId") == primary_task_id), None)
    scope, expected, actual = None, None, None
    if classification == "PATH_PROJECTION":
        scope = _path_projection_scope(associated, identity_location in {"KEY", "BOTH"})
        record_task_id = associated.get("taskId") if isinstance(associated, dict) else None
        if scope is None or capability_id is None or (record_task_id and record_task_id != primary_task_id):
            return {
                "semanticScope": None, "missing": [], "unexpected": [], "contradictions": [],
                "validationExecuted": True, "validationResult": "PASS",
            }
        task_field = {
            "EXACT_CURRENT_PATHS": "exactCurrentPaths", "EXACT_TARGET_PATHS": "exactTargetPaths",
            "ALLOWED_PATHS": "allowedPaths", "OWNED_PATHS": "ownedPaths",
            "REFERENCE_PATHS": "referencePaths", "FORBIDDEN_PATHS": "forbiddenPaths",
        }.get(scope)
        field = next((name for name, candidate_scope in (
            ("exactCurrentPaths", "EXACT_CURRENT_PATHS"), ("paths", "EXACT_CURRENT_PATHS"),
            ("exactTargetPaths", "EXACT_TARGET_PATHS"), ("allowedPaths", "ALLOWED_PATHS"),
            ("ownedPaths", "OWNED_PATHS"), ("referencePaths", "REFERENCE_PATHS"),
            ("forbiddenPaths", "FORBIDDEN_PATHS"),
        ) if candidate_scope == scope and isinstance(associated, dict) and name in associated), None)
        actual = associated if isinstance(associated, list) else (associated.get(field) if field else None)
        expected = (task or {}).get(task_field) if task_field else None
    elif classification == "OWNERSHIP_PROJECTION":
        scope = "CURRENT_OWNER_LABEL"
        actual = associated
        expected = (capability or {}).get("currentOwner")
    else:
        return {
            "semanticScope": None, "missing": [], "unexpected": [], "contradictions": [],
            "validationExecuted": classification != "UNCLASSIFIED_CURRENT_AUTHORITY",
            "validationResult": "PASS" if classification != "UNCLASSIFIED_CURRENT_AUTHORITY" else "FAIL",
        }
    if expected is None or actual is None:
        return {
            "semanticScope": scope, "missing": [expected] if expected is not None else [],
            "unexpected": [actual] if actual is not None else [],
            "contradictions": [{"issue": "scoped canonical authority unavailable", "capabilityId": capability_id, "scope": scope}],
            "validationExecuted": True, "validationResult": "FAIL",
        }
    expected_normalized = _normalized_projection_value("paths", expected) if isinstance(expected, list) else expected
    actual_normalized = _normalized_projection_value("paths", actual) if isinstance(actual, list) else actual
    missing, unexpected = _projection_diff(expected_normalized, actual_normalized)
    contradictions = [] if not missing and not unexpected else [{
        "issue": "scoped projection contradicts canonical authority", "capabilityId": capability_id, "scope": scope,
    }]
    return {
        "semanticScope": scope, "expected": expected_normalized, "actual": actual_normalized,
        "missing": missing, "unexpected": unexpected, "contradictions": contradictions,
        "validationExecuted": True,
        "validationResult": "PASS" if not missing and not unexpected and not contradictions else "FAIL",
    }
...
def _current_authority_reference_universe(authority_classification, overrides, capabilities, tasks, tests, entrypoints):
    ...
    for authority_row in sorted(current_rows, key=lambda row: normalize_rel(row.get("path"))):
        ...
        for record in scan["records"]:
            value = record["_value"]
            classification, rule = _classify_authority_reference(relative, scan["kind"], record["recordLocation"], value, record)
            tokens = sorted({hit["token"] for hit in record["referenceLocations"]})
            unresolved = sorted(token for token in tokens if token not in known_ids and not token.startswith("MR_CAP_001_"))
            scoped_validation = _scoped_reference_validation(classification, record, capabilities, tasks)
            validation_executed = scoped_validation["validationExecuted"]
            validation_result = "PASS" if validation_executed and not unresolved and scoped_validation["validationResult"] == "PASS" else "FAIL"
            artifact_reference_count += 1
            identity_location = _identity_location(record)
            references.append({
                "artifactPath": relative,
                "recordLocation": record["recordLocation"],
                "containingMapLocation": record.get("containingMapLocation"),
                "containingMapKeys": record.get("containingMapKeys") if identity_location in {"KEY", "BOTH"} else [],
                "dictionaryKey": record.get("dictionaryKey"),
                "associatedValue": record.get("associatedValue") if identity_location in {"KEY", "BOTH"} else None,
                "identityLocation": identity_location,
                "authorityStatus": authority_row.get("classification"),
                "capabilityTaskIdentity": tokens,
                "semanticClassification": classification,
                "semanticScope": scoped_validation.get("semanticScope"),
                ...
                "comparisonRule": rule,
                "referenceCount": 1,
                "keyMatchCount": sum(hit.get("identityLocation") == "KEY" for hit in record["referenceLocations"]),
                "valueMatchCount": sum(hit.get("identityLocation") == "VALUE" for hit in record["referenceLocations"]),
                "referenceLocations": record["referenceLocations"],
                "unresolvedIdentities": unresolved,
                "missing": scoped_validation.get("missing", []),
                "unexpected": scoped_validation.get("unexpected", []),
                "contradictions": scoped_validation.get("contradictions", []),
                "expectedScopeValue": scoped_validation.get("expected"),
                "actualScopeValue": scoped_validation.get("actual"),
        ...
        artifacts.append({
            ...
            "traversal": scan.get("traversal") or {},
            "structuredTraversalComplete": scan.get("structuredTraversalComplete"),
        })
    ...
    key_references = [row for row in references if row["identityLocation"] in {"KEY", "BOTH"}]
    value_references = [row for row in references if row["identityLocation"] in {"VALUE", "BOTH"}]
    duplicate_key_value_records = [row for row in references if row["identityLocation"] == "BOTH"]
    structured_artifacts = [row for row in artifacts if row["artifactKind"] == "STRUCTURED"]
    traversal_summary = {
        "structuredArtifactsTotal": len(structured_artifacts),
        "structuredArtifactsTraversed": sum(row.get("structuredTraversalComplete") is True for row in structured_artifacts),
        "dictionaryKeysInspected": sum((row.get("traversal") or {}).get("dictionaryKeysInspected", 0) for row in structured_artifacts),
        "dictionaryValuesTraversed": sum((row.get("traversal") or {}).get("dictionaryValuesTraversed", 0) for row in structured_artifacts),
        "objectsTraversed": sum((row.get("traversal") or {}).get("objectsTraversed", 0) for row in structured_artifacts),
        "listsTraversed": sum((row.get("traversal") or {}).get("listsTraversed", 0) for row in structured_artifacts),
    }
    return {
        ...
        "referenceSummary": {
            ...
            "keyDiscoveredRelevantArtifacts": len({row["artifactPath"] for row in key_references}),
            "valueDiscoveredRelevantArtifacts": len({row["artifactPath"] for row in value_references}),
            "keyMatches": sum(row["keyMatchCount"] for row in references),
            "valueMatches": sum(row["valueMatchCount"] for row in references),
            "keyDiscovered": len(key_references),
            "valueDiscovered": len(value_references),
            "keyValueDuplicateRecords": len(duplicate_key_value_records),
            "discovered": len(references),
            "classified": sum(row["semanticClassification"] != "UNCLASSIFIED_CURRENT_AUTHORITY" for row in references),
            "validated": sum(row["validationExecuted"] for row in references),
            "unclassified": len(unclassified),
            "unvalidated": len(unvalidated),
            "silentlyIgnored": len(set(relevant_paths) - set(inventoried_paths)),
        },
        "traversalSummary": traversal_summary,
...
def architecture_projection_reconciliation(capabilities, change_records, tasks, tests, release_matrix, evidence_rows, receipt_rows, exact_locations, registration_map, authority_classification, entrypoints, overrides=None):
    ...
    for reference in authority["referenceInventory"]:
        if (
            reference.get("authorityStatus") not in CURRENT_AUTHORITY_CLASSES
            or reference.get("semanticClassification") not in {"PATH_PROJECTION", "OWNERSHIP_PROJECTION"}
            or not reference.get("semanticScope")
        ):
            continue
        projection_id = f"{reference['artifactPath']}#{reference['recordLocation']}#{reference['semanticScope']}"
        if projection_id in {row["projectionId"] for row in inventory}:
            continue
        missing = reference.get("missing") or []
        unexpected = reference.get("unexpected") or []
        contradictions = reference.get("contradictions") or []
        status = reference.get("validationResult")
        row = {
            "projectionId": projection_id,
            "filePath": reference["artifactPath"],
            "jsonLocation": reference["recordLocation"],
            "identityLocation": reference.get("identityLocation"),
            "dictionaryKey": reference.get("dictionaryKey"),
            "authorityClassification": reference["authorityStatus"],
            "semanticClassification": reference["semanticClassification"],
            "semanticScope": reference["semanticScope"],
            "semanticFields": [reference["semanticScope"]],
            "canonicalSource": reference["canonicalAuthorityUsed"],
            "comparisonRule": reference["comparisonRule"],
            "expectedTopology": {reference["semanticScope"]: reference.get("expectedScopeValue")},
            "actualTopology": {reference["semanticScope"]: reference.get("actualScopeValue")},
            "missing": missing,
            "unexpected": unexpected,
        ...
        inventory.append(row)
        if status != "PASS":
            issues_by_group[reference["semanticClassification"]].append({
                "projectionId": projection_id, "missing": missing,
                "unexpected": unexpected, "contradictions": contradictions,
            })
    ...
    capability_path_projections = [row for row in inventory if row.get("filePath") == "06 Folder Ownership/CAPABILITY_TO_PATH_MAP.json"]
    folder_ownership_projections = [row for row in inventory if row.get("filePath") == "06 Folder Ownership/FOLDER_OWNERSHIP_MATRIX.json"]
    return {
        "canonicalAuthority": {
        ...
        "capabilityPathProjections": capability_path_projections,
        "folderOwnershipProjections": folder_ownership_projections,
...
def do_strict_validation(overrides=None, validation_mode="CORE_PRE_CHALLENGE", candidate_root=None, temporary_challenge_id=None):
    ...
    traversal_summary = authority_discovery.get("traversalSummary") or {}
    traversal_complete = (
        traversal_summary.get("structuredArtifactsTotal", 0) > 0
        and traversal_summary.get("structuredArtifactsTraversed") == traversal_summary.get("structuredArtifactsTotal")
        and traversal_summary.get("dictionaryKeysInspected", 0) > 0
        and traversal_summary.get("dictionaryValuesTraversed") == traversal_summary.get("dictionaryKeysInspected")
    )
    add(checks, "ARCH-23", "architecture", "Every current-authoritative structured artifact is parsed recursively and every dictionary key and value is traversed", traversal_complete, traversal_summary, {"structuredArtifactsTraversed": "equals total", "dictionaryKeysInspected": "positive", "dictionaryValuesTraversed": "equals dictionaryKeysInspected"}, [] if traversal_complete else [traversal_summary], "instrumented recursive structured-artifact traversal with key/value conservation")
    capability_path_projections = projection_reconciliation.get("capabilityPathProjections") or []
    capability_path_ok = bool(capability_path_projections) and all(row.get("identityLocation") in {"KEY", "BOTH"} and row.get("semanticClassification") == "PATH_PROJECTION" and row.get("semanticScope") == "EXACT_CURRENT_PATHS" and row.get("status") == "PASS" for row in capability_path_projections)
    add(checks, "ARCH-24", "architecture", "CAPABILITY_TO_PATH_MAP is automatically key-discovered and its MR-CAP-001 path projection exactly equals the canonical path contract", capability_path_ok, capability_path_projections, "one or more key-discovered exact-current-path PASS projections", [row for row in capability_path_projections if row.get("status") != "PASS"], "generic key-aware authority discovery followed by canonical scoped path equality")
    folder_ownership_projections = projection_reconciliation.get("folderOwnershipProjections") or []
    folder_ownership_ok = bool(folder_ownership_projections) and all(row.get("identityLocation") in {"KEY", "BOTH"} and row.get("semanticClassification") == "OWNERSHIP_PROJECTION" and row.get("semanticScope") == "CURRENT_OWNER_LABEL" and row.get("status") == "PASS" for row in folder_ownership_projections)
    add(checks, "ARCH-25", "architecture", "FOLDER_OWNERSHIP_MATRIX is automatically key-discovered and its MR-CAP-001 owner label equals current ownership authority", folder_ownership_ok, folder_ownership_projections, "one or more key-discovered current-owner PASS projections", [row for row in folder_ownership_projections if row.get("status") != "PASS"], "generic key-aware authority discovery followed by scoped ownership equality")
    discovery_accounting_ok = (
        discovery_summary.get("keyMatches", 0) > 0
        and discovery_summary.get("valueMatches", 0) > 0
        and discovery_summary.get("discovered")
        == discovery_summary.get("keyDiscovered", 0) + discovery_summary.get("valueDiscovered", 0) - discovery_summary.get("keyValueDuplicateRecords", 0)
    )
    add(checks, "ARCH-26", "architecture", "Key and value discovery share one semantic inventory and key/value duplicates are counted once", discovery_accounting_ok, discovery_summary, {"keyMatches": "positive", "valueMatches": "positive", "discovered": "keyDiscovered + valueDiscovered - keyValueDuplicateRecords"}, [] if discovery_accounting_ok else [discovery_summary], "semantic-record union accounting over KEY, VALUE, and BOTH identity locations")
        ...
        "derived": {
            **derived_counts,
            ...
            "architectureAuthorityDiscovery": {
                ...
                "traversalSummary": (projection_reconciliation.get("authorityDiscovery") or {}).get("traversalSummary"),
                ...
                "capabilityPathProjectionCount": len(projection_reconciliation.get("capabilityPathProjections") or []),
                "folderOwnershipProjectionCount": len(projection_reconciliation.get("folderOwnershipProjections") or []),
                "keyedReferences": [
                    {key: row.get(key) for key in (
                        "artifactPath", "recordLocation", "identityLocation", "dictionaryKey",
                        "semanticClassification", "semanticScope", "validationExecuted", "validationResult",
                    )}
                    for row in ((projection_reconciliation.get("authorityDiscovery") or {}).get("referenceInventory") or [])
                    if row.get("identityLocation") in {"KEY", "BOTH"}
                ],
                "scopedProjectionInventory": projection_reconciliation.get("projectionInventory") or [],
            },
