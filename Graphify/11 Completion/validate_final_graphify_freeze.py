...
def _projection_id(source, pointer):
    if source == "CAPABILITY_REGISTRY":
        if pointer.endswith("/contract/architecturePreservationContract"):
            return "CAPABILITY_REGISTRY.CONTRACT"
        if pointer.endswith("/implementationContract/architecturePreservationContract"):
            return "CAPABILITY_REGISTRY.IMPLEMENTATION_CONTRACT"
        if pointer.endswith("/architectureAuthority"):
            return "CAPABILITY_REGISTRY.ARCHITECTURE_AUTHORITY"
    if source == "CHANGE_LOCATION_REGISTRY":
        if pointer.endswith("/contract/architecturePreservationContract"):
            return "CHANGE_LOCATION_REGISTRY.CONTRACT"
        if pointer.endswith("/architectureAuthority"):
            return "CHANGE_LOCATION_REGISTRY.ARCHITECTURE_AUTHORITY"
    if source == "IMPLEMENTATION_TASKS":
        if pointer.endswith("/contract/architecturePreservationContract"):
            return "IMPLEMENTATION_TASKS.NESTED_CONTRACT"
        if pointer.endswith("/architecturePreservationContract"):
            return "IMPLEMENTATION_TASKS.CANONICAL_CONTRACT"
        return "IMPLEMENTATION_TASKS.TOP_LEVEL"
    if source == "CAPABILITY_EVIDENCE" and pointer.endswith("/topology"):
        return "CAPABILITY_EVIDENCE.TOPOLOGY"
    return f"{source}{pointer}".replace("/", ".")


def architecture_projection_reconciliation(capabilities, change_records, tasks, tests, release_matrix, evidence_rows, receipt_rows, exact_locations, registration_map):
    """Compare the one canonical task topology and every discovered current projection to live source."""
    topology, source_issues = derive_live_architecture_topology()
    capability = next((row for row in capabilities if row.get("capabilityId") == "MR-CAP-001"), None)
    change = next((row for row in change_records if row.get("capabilityId") == "MR-CAP-001"), None)
    task = next((row for row in tasks if row.get("taskId") == "MR-IMPL-001"), None)
    evidence = next((row for row in evidence_rows if row.get("capabilityId") == "MR-CAP-001"), None)
    receipt = next((row for row in receipt_rows if row.get("capabilityId") == "MR-CAP-001"), None)
    exact_location = (exact_locations.get("locations") or {}).get("MR-CAP-001")
    runtime_ids = sorted(
        registration_id for registration_id, row in registration_map.items()
        if "MR-CAP-001" in (row.get("capabilityIds") or [])
    )
    documents = [
        ("CAPABILITY_REGISTRY", "03 Capability Map/CAPABILITY_REGISTRY.json", "/capabilities/MR-CAP-001", capability),
        ("CHANGE_LOCATION_REGISTRY", "04 Exact Location Registry/CHANGE_LOCATION_REGISTRY.jsonl", "/records/MR-CAP-001", change),
        ("IMPLEMENTATION_TASKS", "09 Implementation/IMPLEMENTATION_TASKS.jsonl", "/records/MR-IMPL-001", task),
        ("CAPABILITY_EVIDENCE", "03 Capability Map/CAPABILITY_EVIDENCE.jsonl", "/records/MR-CAP-001", evidence),
        ("CAPABILITY_SOURCE_SEARCH_RECEIPTS", "03 Capability Map/CAPABILITY_SOURCE_SEARCH_RECEIPTS.jsonl", "/records/MR-CAP-001", receipt),
        ("EXACT_LOCATION_REGISTRY", "04 Exact Location Registry/EXACT_LOCATION_REGISTRY.json", "/locations/MR-CAP-001", exact_location),
    ]
    inventory, issues_by_group, seen_ids = [], defaultdict(list), set()
    for source, file_path, base_pointer, document in documents:
        if not isinstance(document, dict):
            continue
        for pointer, projection, fields in _walk_architecture_projections(document, base_pointer):
            projection_id = _projection_id(source, pointer)
            ...
            missing = [
                ...
                for field, result in reconciliations.items() for value in result["missing"]
            ]
            unexpected = [
                {"field": field, "value": value}
                for field, result in reconciliations.items() for value in result["unexpected"]
            ]
            ...
            row = {
                ...
                "authorityClassification": "CURRENT_AUTHORITATIVE",
                "semanticFields": fields,
                "canonicalSource": "09 Implementation/IMPLEMENTATION_TASKS.jsonl#/records/MR-IMPL-001/architecturePreservationContract",
            ...
            if status == "FAIL":
                group = source
                if source == "IMPLEMENTATION_TASKS":
                    group = "IMPLEMENTATION_NESTED" if projection_id == "IMPLEMENTATION_TASKS.NESTED_CONTRACT" else "IMPLEMENTATION_TOP_LEVEL"
                issues_by_group[group].append({"projectionId": projection_id, "missing": missing, "unexpected": unexpected})

    required = {
        "CAPABILITY_REGISTRY": {
            "CAPABILITY_REGISTRY.CONTRACT", "CAPABILITY_REGISTRY.IMPLEMENTATION_CONTRACT",
            "CAPABILITY_REGISTRY.ARCHITECTURE_AUTHORITY",
        },
        "CHANGE_LOCATION_REGISTRY": {
            "CHANGE_LOCATION_REGISTRY.CONTRACT", "CHANGE_LOCATION_REGISTRY.ARCHITECTURE_AUTHORITY",
        },
        "IMPLEMENTATION_TOP_LEVEL": {
            "IMPLEMENTATION_TASKS.TOP_LEVEL", "IMPLEMENTATION_TASKS.CANONICAL_CONTRACT",
        },
        "IMPLEMENTATION_NESTED": {"IMPLEMENTATION_TASKS.NESTED_CONTRACT"},
        "CAPABILITY_EVIDENCE": {"CAPABILITY_EVIDENCE.TOPOLOGY"},
    }
    for group, expected_ids in required.items():
        for projection_id in sorted(expected_ids - seen_ids):
            issues_by_group[group].append({"projectionId": projection_id, "issue": "required current-authoritative projection missing"})
    ...
    canonical_row = next((row for row in inventory if row["projectionId"] == "IMPLEMENTATION_TASKS.CANONICAL_CONTRACT"), None)
    if not canonical_row or canonical_row.get("status") != "PASS":
        canonical_issues.append({"issue": "canonical task topology does not equal source", "projection": canonical_row})
    ...
    if not integration_ids or cross_projection_test_ids != integration_ids:
        test_issues.append({"issue": "integration test projection lacks the canonical cross-projection executable assertion", "integrationTestIds": integration_ids, "assertionTestIds": cross_projection_test_ids})
    inventory.append({
        "projectionId": "REQUIREMENT_TEST_MATRIX.CANONICAL_ASSERTION_BINDING",
        "filePath": "10 Verification/REQUIREMENT_TEST_MATRIX.jsonl",
        "jsonLocation": "/records/MR-CAP-001+MR-IMPL-001",
        "authorityClassification": "CURRENT_AUTHORITATIVE_BINDING",
        "semanticFields": ["executableAssertions"],
        "canonicalSource": "09 Implementation/IMPLEMENTATION_TASKS.jsonl#/records/MR-IMPL-001/architecturePreservationContract",
        "expectedTopology": {"integrationTestIds": integration_ids},
        "actualTopology": {"crossProjectionAssertionTestIds": cross_projection_test_ids},
        "missing": test_issues,
        "unexpected": [],
    ...
    inventory.append({
        ...
        "filePath": "10 Verification/RELEASE_GATE_MATRIX.json",
        "jsonLocation": "/capabilityValidationGates/MR-CAP-001 + /waveGates/WAVE_0",
        "authorityClassification": "CURRENT_AUTHORITATIVE_BINDING",
        "semanticFields": ["requiredTestIds"],
        "canonicalSource": "10 Verification/REQUIREMENT_TEST_MATRIX.jsonl#/records/MR-CAP-001+MR-IMPL-001",
        "expectedTopology": {"requiredCrossProjectionTestIds": cross_projection_test_ids},
        "actualTopology": {"capabilityGateTestIds": capability_gate.get("requiredTestIds") or [], "waveGateTestIds": wave_gate.get("requiredTestIds") or []},
        "missing": gate_issues,
        "unexpected": [],
    ...
    all_issues = [issue for group in sorted(issues_by_group) for issue in issues_by_group[group]]
    return {
        "canonicalAuthority": {
            "projectionId": "IMPLEMENTATION_TASKS.CANONICAL_CONTRACT",
            "filePath": "09 Implementation/IMPLEMENTATION_TASKS.jsonl",
            "jsonLocation": "/records/MR-IMPL-001/architecturePreservationContract",
            "authorityStatus": canonical.get("authorityStatus"),
        ...
        "issues": canonical_issues + all_issues,
    }
